...
if(submit_check('group_submit')){
	...
	if(is_array($selectedids)){
		...
		write_msg('','?m=group&type=corp&success=3&status='.$status.'&page='.$page);
	}
	...
	if(empty($gname)) write_msg('','?m=group&type=corp&ac=detail&error=37&id='.$id);
	if(empty($content)) write_msg('','?m=group&type=corp&ac=detail&error=35&id='.$id);
	...
	if(empty($id)){
		$db -> query("INSERT INTO `{$db_mymps}group` (gname,des,content,pre_picture,picture,cate_id,areaid,gaddress,meetdate,enddate,userid,dateline,glevel) VALUES ('$gname','$des','$content','$pre_picture','$picture','$cate_id','$areaid','$gaddress','$meetdate','$enddate','$s_uid','$timestamp','$glevel')");
		/*积分变化*/
		...
		$url = '?m=group&type=corp&success=8&ac=detail&id='.$id.'&alert=3';
	} else {
		$db -> query("UPDATE `{$db_mymps}group` SET gname = '$gname',des = '$des',content = '$content',cate_id = '$cate_id',areaid = '$areaid',gaddress='$gaddress',picture='$picture',pre_picture='$pre_picture',meetdate = '$meetdate',dateline = '$timestamp' $where AND groupid = '$id'");
	}
	
	write_msg('',$url ? $url : '?m=group&type=corp&success=8&ac=detail&id='.$id);
	
} elseif (submit_check('group_signin_submit')){
	
	empty($selectids) && write_msg('','?m=group&type=corp&ac=signin&error=1&page='.$page);
	$db -> query("DELETE FROM `{$db_mymps}group_signin` {$where} AND signid ".create_in($selectedids));
	unset($selectedids);
	write_msg('','?m=group&type=corp&success=3&ac=signin&page='.$page);
