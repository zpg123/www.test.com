...
if(submit_check('coupon_submit')){
	...
	if(is_array($selectedids)){
		...
		write_msg('','?m=coupon&type=corp&success=3&status='.$status.'&page='.$page);
	}
	...
	if(empty($id)){
		$db -> query("INSERT INTO `{$db_mymps}coupon` (title,des,content,pre_picture,picture,cate_id,areaid,begindate,enddate,userid,dateline,ctype,sup,status) VALUES ('$title','$des','$content','$pre_picture','$picture','$cate_id','$areaid','$begindate','$enddate','$s_uid','$timestamp','$ctype','$sup','$status')");
		/*积分变化*/
		...
		$url = '?m=coupon&type=corp&success=8&ac=detail&id='.$id.'&alert=2';
	} else {
		$db -> query("UPDATE `{$db_mymps}coupon` SET title = '$title',des = '$des',content = '$content',cate_id = '$cate_id',areaid = '$areaid',picture='$picture',pre_picture='$pre_picture',begindate = '$begindate',enddate='$enddate', dateline = '$timestamp' , ctype = '$ctype' , sup = '$sup' , status = '$status' $where AND id = '$id'");
	}
	...
	if($ac == 'list') {
		...
		$status = isset($_GET['status']) ? mhtmlspecialchars($_GET['status']) : '';
