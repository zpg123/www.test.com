...
if(submit_check('base_submit')){
	...
	$istel == 1 && $db -> query("UPDATE `{$db_mymps}information` SET tel = '$tel' {$where}");
	
	if($email == '' && $qq == '' && $tel == '') write_msg('','?m=base&error=11');
	
	$db -> query("UPDATE `{$db_mymps}member` SET qq = '$qq', tel = '$tel' ,mobile='$mobile',cname = '$cname', email = '$email' , sex = '$sex'  {$where}");
