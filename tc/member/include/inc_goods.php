...
if(submit_check('goods_submit')){
	...
	if(is_array($selectedids)){
		...
		write_msg('','?m=goods&type=corp&success=3&status='.$status.'&page='.$page);
	}
	...
	if(empty($goodsname)) write_msg('','?m=goods&type=corp&ac=detail&error=42&id='.$id);
	if(empty($content)) write_msg('','?m=goods&type=corp&ac=detail&error=43&id='.$id);
	if(empty($catid)) write_msg('','?m=goods&type=corp&ac=detail&error=44&id='.$id);
	...
	if(empty($id)){
		$db -> query("INSERT INTO `{$db_mymps}goods` (goodsname,goodsbh,catid,oicq,oldprice,nowprice,content,pre_picture,picture,userid,dateline,tuihuan,jiayi,weixiu,fahuo,cuxiao,baozhang,huoyuan,gift) VALUES ('$goodsname','$goodsbh','$catid','$oicq','$oldprice','$nowprice','$content','$pre_picture','$picture','$s_uid','$timestamp','$tuihuan','$jiayi','$weixiu','$fahuo','$cuxiao','$baozhang','$huoyuan','$gift')");
		$id = $db->insert_id();
		/*积分变化*/
		...
		$url = '?m=goods&type=corp&success=8&ac=detail&id='.$id.'&alert=4';
	} else {
		$db -> query("UPDATE `{$db_mymps}goods` SET goodsname = '$goodsname',catid = '$catid',oicq='$oicq',oldprice = '$oldprice',nowprice = '$nowprice',content = '$content',pre_picture='$pre_picture',picture='$picture',dateline='$timestamp',tuihuan = '$tuihuan',jiayi = '$jiayi',weixiu='$weixiu',fahuo='$fahuo',cuxiao='$cuxiao',baozhang='$baozhang',huoyuan = '$huoyuan',gift='$gift' $where AND goodsid = '$id'");
	}
	
	write_msg('',$url?$url:'?m=goods&type=corp&success=8&ac=detail&id='.$id);
	
	
} elseif (submit_check('goods_order_submit')){

	empty($selectids) && write_msg('','?m=goods&type=corp&ac=order&error=1&page='.$page);
	$db -> query("DELETE FROM `{$db_mymps}goods_order` {$where} AND id ".create_in($selectedids));
	unset($selectedids);
	write_msg('','?m=goods&type=corp&success=3&ac=order&page='.$page);
