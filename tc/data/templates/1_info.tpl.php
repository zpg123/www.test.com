                ...
                <div class="bd_lefter">
                    ...
                    <div class="contact">
                        <ul>
                            ...
                            <li style="<? if(in_array($mymps['title'],array('价格','租金'))) { ?>color:red;<?php } ?>"><span><?=$mymps['title']?>：</span><? if(in_array($mymps['value'],array('0元','0元/月','0元/平米/天','0万元','0.00元','0.00万元'))) { ?>面议<?php } else { ?><?=$mymps['value']?><?php } ?></li>
                            <?php } ?>
                            <?php }} ?>
                            <? if($info['areaname']) { ?><li><span>地址：</span><?=$info['areaname']?> <? if($info['ip'] == 'wap') { ?><font class="font" color="green">手机发布</font><?php } else { ?><font class="font"><?=$info['ip']?></font> <font class="font" color="green"><?=$info['ip2area']?></font><?php } ?></li><?php } ?>
                            <div class="clear"></div>
                            <ul class="contacter">
                            <div class="clear"></div>
                    ...
                    <div class="relates">
                        <ul>
                            <?php $latest_info = mymps_get_infos(6,'','','',$info['catid']); ?>                            <?php if(is_array($latest_info)){foreach($latest_info as $mymps) { ?>                            <? if($mymps['id'] != $info['id']) { ?>
                            <li>
